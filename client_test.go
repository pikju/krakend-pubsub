...
func TestNew_noConfig(t *testing.T) {
	...
	if !strings.HasSuffix(lines[0], "DEBUG: pubsub: subscriber (schema://host): github.com/pikju/krakend-pubsub/subscriber not found in the extra config") {
		t.Error("unexpected first log line:", lines[0])
	}
	if !strings.HasSuffix(lines[1], "DEBUG: pubsub: publisher (schema://host): github.com/pikju/krakend-pubsub/publisher not found in the extra config") {
...
func TestNew_publisher(t *testing.T) {
	...
	lines := strings.Split(buff.String(), "\n")
	if !strings.HasSuffix(lines[0], "DEBUG: pubsub: subscriber (mem://host): github.com/pikju/krakend-pubsub/subscriber not found in the extra config") {
		t.Error("unexpected first log line:", lines[0])
	}
	if lines[1] != "" {
